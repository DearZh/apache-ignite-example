...
public class GridClosureProcessorSelfTest extends GridCommonAbstractTest {
     ...
     */
    private IgniteFuture<?> runAsync(int idx, IgniteRunnable job,  IgnitePredicate<ClusterNode> p) {
     ...
     * @return Future object.
     */
    private IgniteFuture<?> broadcast(int idx, IgniteRunnable job,  IgnitePredicate<ClusterNode> p) {
        assert idx >= 0 && idx < NODES_CNT;
        assert job != null;
