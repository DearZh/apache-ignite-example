...
public abstract class IgniteTxLocalAdapter extends IgniteTxAdapter implements IgniteTxLocalEx {
    ...
    @SuppressWarnings({"RedundantTypeArguments"})
     @Override public GridTuple<CacheObject> peek(
     ...
     */
    private void updateWaitTxs( GridLongList waitTxs) {
        if (waitTxs != null) {
     ...
     * @param expiryPlc Expiry policy.
     */
    void entryExpiry(IgniteTxKey key,  ExpiryPolicy expiryPlc) {
        assert key != null;
    ...
    protected abstract class PostLockClosure1<T> implements IgniteBiClosure<Boolean, Exception, IgniteInternalFuture<T>> {
        ...
        /** {@inheritDoc} */
        @Override public final IgniteInternalFuture<T> apply(Boolean locked,  final Exception e) {
    ...
    protected abstract class PostLockClosure2<T> implements IgniteBiClosure<Boolean, Exception, IgniteInternalFuture<T>> {
        ...
        /** {@inheritDoc} */
        @Override public final IgniteInternalFuture<T> apply(Boolean locked,  Exception e) {
            boolean rollback = true;
