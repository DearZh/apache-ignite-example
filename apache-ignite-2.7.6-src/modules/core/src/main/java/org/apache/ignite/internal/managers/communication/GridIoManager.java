...
public class GridIoManager extends GridManagerAdapter<CommunicationSpi<Serializable>> {
     ...
     * @param lsnr Listener.
     */
    private void changeSystemListener(int idx,  GridMessageListener lsnr) {
        assert Thread.holdsLock(sysLsnrsMux);
    ...
    public void addUserMessageListener( final Object topic,  final IgniteBiPredicate<UUID, ?> p) {
        if (p != null) {
            try {
     ...
     */
    @SuppressWarnings("unchecked")
    public void removeUserMessageListener( Object topic, IgniteBiPredicate<UUID, ?> p) {
        try {
     ...
     */
    @SuppressWarnings("deprecation")
    public boolean removeMessageListener(GridTopic topic,  GridMessageListener lsnr) {
     ...
     */
    @SuppressWarnings({"deprecation", "SynchronizationOnLocalVariableOrMethodParameter"})
    public boolean removeMessageListener(Object topic,  GridMessageListener lsnr) {
        assert topic != null;
    ...
    private class GridUserMessageListener implements GridMessageListener {
         ...
         */
        GridUserMessageListener( Object topic,  IgniteBiPredicate<UUID, Object> predLsnr)
            throws IgniteCheckedException {
            this.topic = topic;
    ...
    private class IoTestFuture extends GridFutureAdapter<List<IgniteIoTestMessage>> {
        ...
        /** {@inheritDoc} */
        @Override public boolean onDone(List<IgniteIoTestMessage> res,  Throwable err) {
