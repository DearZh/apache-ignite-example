...
public class IgniteCacheOffheapManagerImpl implements IgniteCacheOffheapManager {
     ...
     */
     private CacheDataStore partitionData(int p) {
        if (grp.isLocal())
            return locCacheDataStore;
        else {
            GridDhtLocalPartition part = grp.topology().localPartition(p, AffinityTopologyVersion.NONE, false, true);
    ...
    /** {@inheritDoc} */
     @Override public MvccUpdateResult mvccLock(GridCacheMapEntry entry,
        MvccSnapshot mvccSnapshot) throws IgniteCheckedException {
        if (entry.detached() || entry.isNear())
            return null;
    ...
    @Override  public CacheDataRow read(GridCacheMapEntry entry)
        throws IgniteCheckedException {
    ...
    /** {@inheritDoc} */
     @Override public CacheDataRow read(GridCacheContext cctx, KeyCacheObject key)
        throws IgniteCheckedException {
    ...
    /** {@inheritDoc} */
     @Override public CacheDataRow mvccRead(GridCacheContext cctx, KeyCacheObject key, MvccSnapshot ver)
        throws IgniteCheckedException {
        assert ver != null;
     ...
     * @return Data store.
     */
     private CacheDataStore dataStore(GridCacheContext cctx, KeyCacheObject key) {
        if (grp.isLocal())
            return locCacheDataStore;
     ...
     */
     protected IgniteHistoricalIterator historicalIterator(CachePartitionPartialCountersMap partCntrs, Set<Integer> missing)
        throws IgniteCheckedException {
        return null;
    }
    ...
    protected class CacheDataStoreImpl implements CacheDataStore {
         ...
         * @throws IgniteCheckedException If failed.
         */
        private boolean canUpdateOldRow(GridCacheContext cctx,  CacheDataRow oldRow, DataRow dataRow)
            throws IgniteCheckedException {
        ...
        /** {@inheritDoc} */
        @Override public int cleanup(GridCacheContext cctx,  List<MvccLinkAwareSearchRow> cleanupRows)
            throws IgniteCheckedException {
            int res = 0;
         ...
         * @throws IgniteCheckedException If failed.
         */
        private void finishUpdate(GridCacheContext cctx, CacheDataRow newRow,  CacheDataRow oldRow)
            throws IgniteCheckedException {
            if (oldRow == null)
                incrementSize(cctx.cacheId());
         ...
         * @throws IgniteCheckedException If failed.
         */
        private void updatePendingEntries(GridCacheContext cctx, CacheDataRow newRow,  CacheDataRow oldRow)
            throws IgniteCheckedException
        {
         ...
         * @throws IgniteCheckedException If failed.
         */
        private void finishRemove(GridCacheContext cctx, KeyCacheObject key,  CacheDataRow oldRow) throws IgniteCheckedException {
            if (oldRow != null) {
                clearPendingEntries(cctx, oldRow);
         ...
         * @throws IgniteCheckedException If failed.
         */
        private void afterRowFound( CacheDataRow row, KeyCacheObject key) throws IgniteCheckedException {
            if (row != null) {
                row.key(key);
         ...
         */
        private int valueLength(GridCacheContext cctx,  CacheObject val) {
            if (val == null)
                return 0;
        ...
        private class MvccPutIfAbsentClosure extends MvccDataRow implements IgniteTree.InvokeClosure<CacheDataRow> {
            ...
            /** {@inheritDoc} */
            @Override public void call( CacheDataRow old) throws IgniteCheckedException {
                if (old == null) {
                    op = PUT;
