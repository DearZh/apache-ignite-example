...
abstract class IgfsAbstractOutputStream extends IgfsOutputStream {
     ...
     */
    IgfsAbstractOutputStream(IgfsContext igfsCtx, IgfsPath path, int bufSize,  IgfsFileWorkerBatch batch) {
        synchronized (mux) {
            this.path = path;
     ...
     */
    protected void checkClosed( DataInput in, int len) throws IOException {
