...
public class GridCacheMvccManager extends GridCacheSharedManagerAdapter {
     ...
     * @return Future.
     */
     public IgniteInternalFuture<?> atomicFuture(long futId) {
     ...
     */
     public IgniteInternalFuture<?> removeAtomicFuture(long futId) {
     ...
     */
    @SuppressWarnings({"unchecked"})
     public GridCacheVersionedFuture<?> versionedFuture(GridCacheVersion ver, IgniteUuid futId) {
     ...
     */
    @SuppressWarnings({"unchecked"})
     public Collection<GridCacheVersionedFuture<?>> futuresForVersion(GridCacheVersion ver) {
        Collection<GridCacheVersionedFuture<?>> futs = this.verFuts.get(ver);
     ...
     */
     public GridCacheMvccCandidate explicitLock(IgniteTxKey key,  GridCacheVersion ver) {
        for (GridCacheExplicitLockSpan span : pendingExplicit.values()) {
    ...
    private IgniteInternalFuture<?> finishLocks(
         final IgnitePredicate<GridDistributedCacheEntry> filter,
        AffinityTopologyVersion topVer
    ) {
    ...
    private class FinishLockFuture extends GridFutureAdapter<Object> {
         ...
         */
        @SuppressWarnings({"SynchronizationOnLocalVariableOrMethodParameter"})
        void recheck( GridCacheEntryEx entry) {
            if (entry == null)
                return;
