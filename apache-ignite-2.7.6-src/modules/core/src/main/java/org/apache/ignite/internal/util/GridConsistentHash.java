...
public class GridConsistentHash<N> {
     ...
     */
    public GridConsistentHash( Object affSeed) {
        this(null, affSeed);
    }
     ...
     * @param affSeed Affinity seed (will be used as key prefix for hashing).
     */
    public GridConsistentHash( Comparator<N> nodesComp,  Object affSeed) {
     ...
     * @return Node for key, or {@code null} if node was not found.
     */
     public N node( Object key,  IgnitePredicate<N>... p) {
     ...
     */
    public List<N> nodes( Object key, int cnt) {
     ...
     * @return List containing adjacent nodes for given key.
     */
    public List<N> nodes( Object key, int cnt,  Collection<N> inc) {
     ...
     * @return List containing adjacent nodes for given key.
     */
    public List<N> nodes( Object key, int cnt,  final Collection<N> inc,
         final Collection<N> exc) {
     ...
     * @return List containing adjacent nodes for given key.
     */
    public List<N> nodes( Object key, int cnt,  IgnitePredicate<N>... p) {
        A.ensure(cnt >= 0, "cnt >= 0");
     ...
     */
    public boolean belongs( Object key, N node) {
        A.notNull(node, "node");
     ...
     */
    public boolean belongs( Object key,  Collection<N> nodes) {
        if (F.isEmpty(nodes))
            return false;
     ...
     */
    public boolean belongs( Object key, int cnt, N node) {
     ...
     */
    public boolean belongs( Object key, int cnt,  Collection<N> nodes) {
        if (F.isEmpty(nodes))
            return false;
