...
public class GridClosureProcessor extends GridProcessorAdapter {
     ...
     */
    public ComputeTaskInternalFuture<?> runAsync(GridClosureCallMode mode,  Collection<? extends Runnable> jobs,
         Collection<ClusterNode> nodes,  String execName) {
    ...
    public <R> ComputeTaskInternalFuture<R> callAsync(GridClosureCallMode mode,
         Callable<R> job,  Collection<ClusterNode> nodes) {
     ...
     */
    public <R> ComputeTaskInternalFuture<R> affinityCall(Collection<String> cacheNames,
        int partId,
        Callable<R> job,
         Collection<ClusterNode> nodes,
         String execName) throws IgniteCheckedException {
     ...
     * @throws IgniteCheckedException If failed.
     */
    public ComputeTaskInternalFuture<?> affinityRun(Collection<String> cacheNames,
        int partId,
        Runnable job,
         Collection<ClusterNode> nodes,
         String execName) throws IgniteCheckedException {
     ...
     * @return Grid future for execution result.
     */
    public <T, R> IgniteInternalFuture<Collection<R>> broadcast(IgniteClosure<T, R> job,  T arg,
         Collection<ClusterNode> nodes,  String execName) {
        busyLock.readLock();
    ...
    public <T, R1, R2> ComputeTaskInternalFuture<R2> callAsync(IgniteClosure<T, R1> job,
        Collection<? extends T> args, IgniteReducer<R1, R2> rdc,  Collection<ClusterNode> nodes,
         String execName) {
        busyLock.readLock();
     ...
     */
    public IgniteInternalFuture<?> runLocal( final Runnable c, byte plc)
     ...
     * @throws IgniteCheckedException Thrown in case of any errors.
     */
    private <R> IgniteInternalFuture<R> callLocal( final Callable<R> c, boolean sys) throws IgniteCheckedException {
     ...
     * @throws IgniteCheckedException Thrown in case of any errors.
     */
    public <R> IgniteInternalFuture<R> callLocal( final Callable<R> c, byte plc)
        throws IgniteCheckedException {
        if (c == null)
            return new GridFinishedFuture<>();
     ...
     * @return Job.
     */
    private static <T, R> ComputeJob job(final IgniteClosure<T, R> job,  final T arg) {
        A.notNull(job, "job");
    ...
    private abstract static class TaskNoReduceAdapter<T> extends GridPeerDeployAwareTaskAdapter<T, Void> {
         ...
         */
        protected TaskNoReduceAdapter( GridPeerDeployAware pda) {
            super(pda);
        }
    ...
    private static class T4 extends TaskNoReduceAdapter<Void> implements GridNoImplicitInjection, AffinityTask {
        ...
        /** {@inheritDoc} */
         @Override public Collection<String> affinityCacheNames() {
    ...
    private static class T8<T, R> extends GridPeerDeployAwareTaskAdapter<Void, R> implements GridNoImplicitInjection {
         ...
         * @param arg Optional job argument.
         */
        private T8(IgniteClosure<T, R> job,  T arg) {
            super(U.peerDeployAware(job));
    ...
    private class T11<T, R> extends GridPeerDeployAwareTaskAdapter<T, Collection<R>>
        ...
        /** {@inheritDoc} */
        @Override public Map<? extends ComputeJob, ClusterNode> map(List<ClusterNode> subgrid,  T arg) {
