...
class GridTaskWorker<T, R> extends GridWorker implements GridTimeoutObject {
    ...
    GridTaskWorker(
        ...
        GridTaskEventListener evtLsnr,
         Map<GridTaskThreadContextKey, Object> thCtx,
        UUID subjId) {
    ...
    @SuppressWarnings({"CatchGenericClass"})
     private ComputeJobResultPolicy result(final ComputeJobResult jobRes, final List<ComputeJobResult> results) {
        ...
        return U.wrapThreadLoader(dep.classLoader(), new CO<ComputeJobResultPolicy>() {
             @Override public ComputeJobResultPolicy apply() {
                try {
    ...
    private void recordJobEvent(int evtType, IgniteUuid jobId, ClusterNode evtNode,
         ComputeJobResultPolicy plc, String msg) {
        if (!internal && ctx.event().isRecordable(evtType)) {
     ...
     * @param e Exception.
     */
    void finishTask( R res,  Throwable e) {
     ...
     */
    void finishTask( R res,  Throwable e, boolean cancelChildren) {
