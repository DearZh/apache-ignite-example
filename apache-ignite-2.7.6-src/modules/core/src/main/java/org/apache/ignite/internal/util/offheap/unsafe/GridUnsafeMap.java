...
public class GridUnsafeMap implements GridOffHeapMap {
    ...
    GridUnsafeMap(int part, int concurrency, float load, long initCap, LongAdder totalCnt, GridUnsafeMemory mem,
        GridUnsafeLru lru,  GridOffHeapEvictListener evictLsnr, GridUnsafeLruPoller lruPoller) {
        this.part = part;
    ...
    /** {@inheritDoc} */
     @Override public IgniteBiTuple<Long, Integer> valuePointer(int hash, byte[] keyBytes) {
...
//                            bin.add(F.t(Entry.readKeyBytes(entryAddr, mem), Entry.readValueBytes(entryAddr, mem)));
         ...
         */
        @SuppressWarnings("TooBroadScope")
        byte[] remove(int hash, byte[] keyBytes, boolean retval,  IgniteBiPredicate<Long, Integer> p) {
            int relSize = 0;
            long relAddr = 0;
            long qAddr = 0;
