...
public final class ScanQuery<K, V> extends Query<Cache.Entry<K, V>> {
     ...
     */
    public ScanQuery( IgniteBiPredicate<K, V> filter) {
        this(null, filter);
    }
     ...
     * @param filter Filter. If {@code null} then all entries will be returned.
     */
    public ScanQuery( Integer part,  IgniteBiPredicate<K, V> filter) {
        setPartition(part);
        setFilter(filter);
    }
     ...
     */
    public ScanQuery<K, V> setFilter( IgniteBiPredicate<K, V> filter) {
        this.filter = filter;
     ...
     * @return {@code this} for chaining.
     */
    public ScanQuery<K, V> setPartition( Integer part) {
        this.part = part;
