...
public class DataStreamProcessor<K, V> extends GridProcessorAdapter {
     ...
     * @return Data loader.
     */
    public DataStreamerImpl<K, V> dataStreamer( String cacheName) {
        if (!busyLock.enterBusy())
     ...
     */
    private void sendResponse(UUID nodeId, Object resTopic, long reqId,  Throwable err,
        boolean forceLocDep) {
        byte[] errBytes;
     ...
     * @return IO policy.
     */
    public static byte ioPolicy( IgniteClosure<ClusterNode, Byte> rslvr, ClusterNode node) {
        assert node != null;
