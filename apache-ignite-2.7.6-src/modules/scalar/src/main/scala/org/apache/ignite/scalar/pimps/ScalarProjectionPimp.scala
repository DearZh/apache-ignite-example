...
class ScalarProjectionPimp[A <: ClusterGroup] extends PimpedType[A] with Iterable[ClusterNode]
     ...
     */
    private def forPredicate( p: NF): ClusterGroup =
     ...
     */
    def nodes$( p: NF): Seq[ClusterNode] =
     ...
     */
    def remoteNodes$( p: NF = null): Seq[ClusterNode] =
     ...
     */
    def call$[R]( s: Seq[Call[R]],  p: NF): Seq[R] =
     ...
     * @return Sequence of result values from all nodes where given closures were executed or `null` (see above).
     */
    def callSafe[R]( s: Seq[Call[R]], dflt: () => Seq[R],  p: NF): Seq[R] = {
        assert(dflt != null)
     ...
     * @see `org.apache.ignite.cluster.ClusterGroup.call(...)`
     */
    def call$[R]( s: Call[R],  p: NF): Seq[R] =
        call$(Seq(s), p)
     ...
     * @see `org.apache.ignite.cluster.ClusterGroup.call(...)`
     */
    def callSafe[R]( s: Call[R], dflt: () => Seq[R],  p: NF): Seq[R] = {
        assert(dflt != null)
     ...
     * @see `org.apache.ignite.cluster.ClusterGroup.run(...)`
     */
    def runSafe( s: Seq[Run],  dflt: Run,  p: NF) {
        try {
            run$(s, p)
        }
        catch {
     ...
     * @see `org.apache.ignite.cluster.ClusterGroup.run(...)`
     */
    def runSafe( s: Run,  dflt: Run,  p: NF) {
        try {
            run$(s, p)
        }
        catch {
     ...
     * @see `org.apache.ignite.cluster.ClusterGroup.call(...)`
     */
    def callAsync$[R]( s: Seq[Call[R]],  p: NF):
     ...
     * @see `org.apache.ignite.cluster.ClusterGroup.call(...)`
     */
    def #?[R]( s: Seq[Call[R]],  p: NF): IgniteFuture[java.util.Collection[R]] = {
        callAsync$(s, p)
    }
     ...
     * @see `org.apache.ignite.cluster.ClusterGroup.call(...)`
     */
    def callAsync$[R]( s: Call[R],  p: NF): IgniteFuture[java.util.Collection[R]] = {
        callAsync$(Seq(s), p)
    }
     ...
     * @see `org.apache.ignite.cluster.ClusterGroup.call(...)`
     */
    def #?[R]( s: Call[R],  p: NF): IgniteFuture[java.util.Collection[R]] = {
        callAsync$(s, p)
    }
     ...
     * @see `org.apache.ignite.cluster.ClusterGroup.call(...)`
     */
    def runAsync$( s: Seq[Run],  p: NF): IgniteFuture[_] = {
        val comp = value.ignite().compute(forPredicate(p))
     ...
     * @see `org.apache.ignite.cluster.ClusterGroup.call(...)`
     */
    def *?( s: Seq[Run],  p: NF): IgniteFuture[_] = {
        runAsync$(s, p)
    }
     ...
     * @see `org.apache.ignite.cluster.ClusterGroup.run(...)`
     */
    def runAsync$( s: Run,  p: NF): IgniteFuture[_] = {
        runAsync$(Seq(s), p)
    }
     ...
     */
    def reduceAsync$[R1, R2](s: Seq[Call[R1]], r: Seq[R1] => R2,  p: NF): IgniteFuture[R2] = {
     ...
     * @see `org.apache.ignite.cluster.ClusterGroup.reduce(...)`
     */
    def @?[R1, R2](s: Seq[Call[R1]], r: Seq[R1] => R2,  p: NF): IgniteFuture[R2] = {
        reduceAsync$(s, r, p)
    }
     ...
     * @see `org.apache.ignite.cluster.ClusterGroup.reduce(...)`
     */
    def reduce$[R1, R2]( s: Seq[Call[R1]],  r: Seq[R1] => R2,  p: NF): R2 =
     ...
     * @see `org.apache.ignite.cluster.ClusterGroup.reduce(...)`
     */
    def reduceSafe[R1, R2]( s: Seq[Call[R1]],  r: Seq[R1] => R2,
        dflt: () => R2,  p: NF): R2 = {
        assert(dflt != null)
     ...
     * @see `org.apache.ignite.cluster.ClusterGroup.reduce(...)`
     */
    def @<[R1, R2]( s: Seq[Call[R1]],  r: Seq[R1] => R2,  p: NF): R2 =
        reduceAsync$(s, r, p).get
     ...
     */
    def affinityRun$(cacheName: String,  affKey: Any,  r: Run,  p: NF) {
     ...
     * @throws IgniteFutureCancelledException Subclass of `IgniteException` thrown if computation was cancelled.
     */
    def affinityRunAsync$(cacheName: String,  affKey: Any,  r: Run,
         p: NF): IgniteFuture[_] = {
        val comp = value.ignite().compute(forPredicate(p))
