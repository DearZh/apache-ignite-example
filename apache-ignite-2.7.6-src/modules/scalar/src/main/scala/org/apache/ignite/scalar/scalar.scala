...
object scalar extends ScalarConversions {
     ...
     * @param cacheName Name of the cache to get.
     */
    @inline def cache$[K, V]( igniteInstanceName: String,
     ...
     */
    @inline def ignite$( name: String): Option[Ignite] =
        try {
     ...
     */
    def stop( name: String, cancel: Boolean) =
        if (isStarted(name))
     ...
     */
    def start( springCfgPath: String): Ignite = {
