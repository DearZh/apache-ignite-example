...
with Iterable[Cache.Entry[K, V]] with Ordered[IgniteCache[K, V]] {
     ...
     */
    private def unwrap( p: Seq[EntryPred]): Seq[IgnitePredicate[Cache.Entry[K, V]]] =
        if (p == null)
            null
        else
     ...
     */
    def putAll$(kv1: (K, V), kv2: (K, V),  kvs: (K, V)*) {
