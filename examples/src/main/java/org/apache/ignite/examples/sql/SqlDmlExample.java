...
public class SqlDmlExample {
    ...
    public static void main(String[] args) throws Exception {
        try (Ignite ignite = Ignition.start("examples/config/example-ignite.xml")) {
            ...
            ignite.destroyCache("AA");

            /**
             * 当使用非DDL方式创建表和索引时，他们所属于的模式名默认为CacheConfiguration中所配置的缓存名；
             * 也或者通过CacheConfiguration.setSqlSchema修改对应的schema名称；
             * 此处采用的注解方式进行的表和索引创建，所以默认模式名为：ORG_CACHE
             */
            CacheConfiguration<Long, Organization> orgCacheCfg = new CacheConfiguration<>(ORG_CACHE);
            orgCacheCfg.setIndexedTypes(Long.class, Organization.class);

            CacheConfiguration<Long, Person> personCacheCfg = new CacheConfiguration<>(PERSON_CACHE);
            //注册索引类型：通知Ignite当前Person.class中那些类型的字段，应该被建立索引；支持数组传值
